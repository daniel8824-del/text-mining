...
class TextMiningAnalysis:
    ...
    def create_word_cloud(self, width=800, height=400, font_path=None):
        ...
        # 모든 단어 합치기
        all_words = " ".join([" ".join(doc) for doc in self.tokenized_corpus])
        
        # 폰트 경로가 없으면 나눔고딕 경로 자동 탐색
        if font_path is None:
            # 가능한 폰트 경로 목록 확장
            possible_paths = [
                '/var/app/current/fonts/NanumGothic.ttf',              # EB 배포 환경 (1순위)
                '/usr/share/fonts/truetype/nanum/NanumGothic.ttf',     # 일반적인 Linux 설치 경로
                '/usr/share/fonts/korean/NanumGothic.ttf',             # 추가 Linux 설치 경로
                os.path.dirname(os.path.abspath(__file__)) + '/fonts/NanumGothic.ttf',  # 절대 경로
            ...
            for idx, path in enumerate(possible_paths):
                if os.path.exists(path):
                    ...
                    print(f"[X] {idx+1}. 폰트 경로 없음: {path}")
            print("--- 폰트 경로 검색 종료 ---")
            
            # 마지막 수단: 시스템에 설치된 모든 폰트에서 NanumGothic 검색
            if font_path is None:
                try:
                    ...
                    nanum_fonts = [f for f in system_fonts if 'nanum' in f.lower()]
                    if nanum_fonts:
                        print(f"Nanum 관련 폰트 파일 발견: {nanum_fonts[0]}")
                        font_path = nanum_fonts[0]
                except Exception as e:
                    print(f"시스템 폰트 검색 오류: {e}")
        
        try:
            # 폰트 파일이 존재하는지 한번 더 확인
            if font_path and os.path.exists(font_path):
                ...
                try:
                    with open(font_path, 'rb') as f:
                        # 파일 시작 부분 읽어보기 (권한 테스트)
                        f.read(10)
                        print(f"폰트 파일 읽기 테스트 성공")
                except Exception as e:
                    print(f"폰트 파일 읽기 테스트 실패: {e}")
                    # 권한 재설정 시도
                    try:
            ...
            # 폴백: 영어 단어만 사용하여 워드클라우드 생성 시도
            try:
                print("영어 단어만 사용해 워드클라우드 생성을 시도합니다.")
                # 영어 정규표현식 패턴
                import re
                english_words = re.sub(r'[^a-zA-Z\s]', '', all_words)
                
                # 영어 단어가 충분한지 확인
                if len(english_words.split()) < 10:
                    print("영어 단어가 부족합니다. 임의 단어를 추가합니다.")
                    english_words += " text mining analysis word cloud example test data python"
                ...
                wordcloud = WordCloud(
                    ...
                    contour_width=1,
                    contour_color='steelblue'
                ).generate(english_words)
            except Exception as e2:
                print(f"영어 워드클라우드 생성도 실패: {e2}")
                return None
